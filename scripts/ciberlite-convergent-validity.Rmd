...
ufs::safeRequire('tidyverse');
ufs::safeRequire('gridExtra');
ufs::safeRequire('readODS');
...
## Validity and internal consistency: full measurement instruments {.tabset .tabset-pills}

```{r validity-and-internal-consistency-full, results="asis"}

for (i in seq_along(behaviors)) {
  ufs::heading("Behavior: ", behaviors[i], headingLevel = 3);
  
  intention_varName <-
    names(detStruct_original[[i]]$children);
  determinant_varNames <-
    names(detStruct_original[[i]][[intention_varName]]$children);
  
  currentVarNames <-
    grep(paste0(
      behaviors[i],
      detStruct_original[[i]][[intention_varName]]$selection),
      names(dat), value=TRUE);
  
  ufs::heading("Intention", headingLevel = 4);
  
  cat("\n\nEigenvalues:",
      vecTxt(round(eigen(cor(dat[, currentVarNames],
                             use='complete.obs'))$values, 2)),
      "\n\n");
  
  print(rosetta::rosettaReliability_partial(
    rosetta::reliability(
      data = dat,
  ...
  for (j in determinant_varNames) {
    
    ufs::heading(j, headingLevel = 4);
    
    currentVarNames <-
      grep(paste0(
        behaviors[i],
        detStruct_original[[i]][[intention_varName]][[j]]$selection),
        names(dat), value=TRUE);
...
## Validity and internal consistency: brief measurement instruments, subset of long {.tabset .tabset-pills}

```{r validity-and-internal-consistency-brief-subset, results="asis"}

for (i in seq_along(behaviors)) {
  ufs::heading("Behavior: ", behaviors[i], headingLevel = 3);
  
  intention_varName <-
    names(detStruct_short[[i]]$children);
  determinant_varNames <-
    names(detStruct_short[[i]][[intention_varName]]$children);
  ...
  for (j in determinant_varNames) {
    ...
    currentVarNames <-
      grep(paste0(
        behaviors[i],
        detStruct_short[[i]][[intention_varName]][[j]]$selection),
        names(dat), value=TRUE);

    cat("\n\nEigenvalues: ",
        vecTxt(round(eigen(cor(dat[dat[, paste0(behaviors[i], "Version")] == "long",
                               currentVarNames],
                               use='complete.obs'))$values, 2)),
        "\n\n");
    ...
    print(rosetta::rosettaReliability_partial(
      rosetta::reliability(
        data = dat[dat[paste0(behaviors[i], "Version")] == "long", ],
        items = currentVarNames,
        scatterMatrix = TRUE,
        headingLevel = 5
      )
...
## Validity and internal consistency: brief measurement instruments, isolated {.tabset .tabset-pills}

```{r validity-and-internal-consistency-brief-isolated, results="asis"}
...
for (i in seq_along(behaviors)) {
  ...
  for (j in determinant_varNames) {
    ...
    cat("\n\nEigenvalues: ",
        vecTxt(round(eigen(cor(dat[dat[paste0(behaviors[i], "Version")] == "short",
                               currentVarNames],
                               use='complete.obs'))$values, 2)),
        "\n\n");
    ...
    print(rosetta::rosettaReliability_partial(
      rosetta::reliability(
        data = dat[dat[, paste0(behaviors[i], "Version")] == "short", ],
        items = currentVarNames,
        scatterMatrix = TRUE,
        headingLevel = 5
      )
...
for (i in seq_along(behaviors)) {
  ...
  corTestObjects[[behaviors[i]]] <-
    lapply(determinants,
           function(v) {
             return(cor.test(dat[, FindNode(detStruct_original[[i]],
                                            paste0(v, "_original"))$scaleVarName],
                             dat[, FindNode(detStruct_short[[i]],
  ...
  corMatrixObjects_full[[behaviors[i]]] <-
    associationMatrix(dat, x = fullVarNames);
  ...
  corMatrixObjects_lean[[behaviors[i]]] <-
    associationMatrix(dat, x = leanVarNames);

  cat0("\n### Correlations between full and lean operationalisations\n\n");
  for (j in names(corTestObjects[[behaviors[i]]])) {
  cat0("\n- **", j, "**: ",
       formatCI(corTestObjects[[behaviors[i]]][[j]]$conf.int, noZero=TRUE),
       ", point estimate = ", formatR(corTestObjects[[behaviors[i]]][[j]]$estimate),
  ...
  pander(corMatrixObjects_full[[behaviors[i]]]);
  ...
  pander(corMatrixObjects_lean[[behaviors[i]]]);
  
  cat0("\n### Q (effect size for difference between correlations)\n\n");
  
  fullZ <- convert.r.to.fisherz(corMatrixObjects_full[[i]]$output$raw$es);
  leanZ <- convert.r.to.fisherz(corMatrixObjects_lean[[i]]$output$raw$es);
  diffQ <- fullZ - leanZ;
  ZforQ <- diffQ / sqrt((1 / (sampleSize_full - 3)) +
                        (1 / (sampleSize_lean - 3)));
  pforQ <- 2*ifelse(ZforQ < 0, pnorm(ZforQ), 1-pnorm(ZforQ));
  
  qMatrix <- paste0("q=", formatR(diffQ), "\n",
                    formatPvalue(pforQ));
