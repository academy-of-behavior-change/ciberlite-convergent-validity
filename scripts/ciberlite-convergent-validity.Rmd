...
########################################################################
### Configure dataset characteristics
########################################################################
      ...
      return(
        behaviorchange::determinantStructure(
          ...
          behaviorchange::determinantVar(
            ...
            behaviorchange::determinantVar(
              "perceivedNorm_original",
              "_pn[a-zA-Z]+$"
            ),
            behaviorchange::determinantVar(
              "perceivedBehavioralControl_original",
              "_pbc[a-zA-Z]+$"
            )
          )
        )
      ...
      return(
        behaviorchange::determinantStructure(
          ...
          behaviorchange::determinantVar(
            ...
            behaviorchange::determinantVar(
              "perceivedBehavioralControl_short",
...
```{r determinant-structure-variable-names, results="asis"}
...
```{r validity-and-internal-consistency-full, results="asis"}

validityObject <- function(dat, items, headingLevel = 5) {
  return(
    list(
      eigen = eigen(cor(dat[, items],
                        use='complete.obs'))$values,
      reliability = rosetta::reliability(
                      data = dat,
                      items = items,
                      scatterMatrix = TRUE,
...
validitySummary <- function(type,
                            validityObject) {
  if (length(validityObject$eigen) > 2) {
    return(
      data.frame(
        type = type,
        eigen = ufs::vecTxt(round(validityObject$eigen[1:2], 2)),
        omega = ufs::formatCI(
          validityObject$reliability$scaleStructure$output$omega.ci,
          noZero=TRUE
        ),
        H = ufs::formatR(
          validityObject$reliability$scaleStructure$output$coefficientH
        ),
        glb = ufs::formatR(
          validityObject$reliability$scaleStructure$output$glb
        ),
        alpha = ufs::formatCI(
          validityObject$reliability$scaleStructure$output$alpha.ci,
          noZero=TRUE
        ),
        spBr = "",
        n = nrow(validityObject$reliability$data)
      )
    );
  } else {
    return(
      data.frame(
        ...
        glb = "",
        alpha = ufs::formatR(
          validityObject$reliability$scaleStructure$output$cronbach.alpha
        ),
        spBr = ufs::formatR(
          validityObject$reliability$scaleStructure$output$spearman.brown
        ),
        n = nrow(validityObject$reliability$data)
      )
    );
  ...
  res <-
    lapply(
      ...
      function(i) {
        ...
        ### select rows from the Df
        tmpDf <-
          dat[dat[, paste0(behaviors[i], "Version")] == version, ];
        
        intention_varName <-
          names(detStruct[[i]]$children);
        determinant_varNames <-
          names(detStruct[[i]][[intention_varName]]$children);
          ...
          lapply(
            ...
            function(j) {
              currentVarNames <-
                unname(
                  unlist(
                    detStruct[[i]][[intention_varName]][[j]]$varNames
                  )
                );
              res <- validityObject(
                dat=tmpDf,
                items=currentVarNames,
                headingLevel=5);
              return(
                list(
                  object = res,
                  summary = validitySummary(type=type,
                                            validityObject=res)
                )
              );
            }
          );
        names(res) <- determinant_varNames;
        currentVarNames <-
          unlist(unname(unclass(detStruct[[i]][[intention_varName]]$varNames)));
        print(currentVarNames);
        if (length(currentVarNames) > 1) {
          res$intention <-
            list(
          ...
          res$intention$summary <-
            validitySummary(type=type,
                            validityObject=res$intention$object);
        } else {
          res$intention <- list(
            object = NULL,
            summary = res[[1]]$summary
          );
          res$intention$summary[, 2:7] <- "";
        }

        ### Reorder to place intention first
        res <-
          res[c('intention',
                determinant_varNames)];
...
validity <-
  list(full = validityObjects(dat = dat,
                              version = "long",
                              behaviors = behaviors,
                              detStruct = detStruct_original,
                              type="full"),
       subset = validityObjects(dat = dat,
                                version = "long",
                                behaviors = behaviors,
                                detStruct = detStruct_short,
                                type="subset"),
       short = validityObjects(dat = dat,
                               version = "short",
                               behaviors = behaviors,
                               detStruct = detStruct_short,
                               type = "short")
              ...
              function(behavior) {
                return(
                  do.call(
                    rbind,
                    lapply(
                      names(validity[[type]][[behavior]]),
                      function(determinant) {
                        return(
                          cbind(
                            data.frame(determinant = determinant,
                                       behavior = behavior),
                            validity[[type]][[behavior]][[determinant]]$summary
                          )
                        );
                      }
                    )
...
validityDf$determinant_original <-
  validityDf$determinant;
validityDf$determinant <-
  gsub("_.*",
       "",
       validityDf$determinant_original);
...
```{r validity-intention}
kableExtra::kable_styling(
  knitr::kable(
    validityDf[
      validityDf$determinant == 'intention',
      2:9
    ]
  )
);
...
kableExtra::kable_styling(
  knitr::kable(
    validityDf[
      validityDf$determinant == 'attitude',
      2:9
    ]
  )
);
...
### Validity: perceived norm

```{r validity-perceivedNorm}
kableExtra::kable_styling(
  knitr::kable(
    validityDf[
      validityDf$determinant == 'perceivedNorm',
      2:9
    ]
  )
);
...
### Validity: perceived behavioral control

```{r validity-perceivedBehavioralControl}
kableExtra::kable_styling(
  knitr::kable(
    validityDf[
      validityDf$determinant == 'perceivedBehavioralControl',
      2:9
    ]
  )
);
...
```{r main-results, results="asis"}

### Loop through behaviors
corTestObjects <- list();
corMatrixObjects_full <- list();
corMatrixObjects_lean <- list();
determinants <- c("intention", "attitude", "perceivedNorm", "perceivedBehavioralControl");
all_p_values <- c();
for (i in seq_along(behaviors)) {

  ### Title for this behavior
  ufs::cat0("\n\n## ", behaviors[i], "\n");

  ### Correlation between short and long versions
  corTestObjects[[behaviors[i]]] <-
    lapply(
      determinants,
      function(v) {
        return(
          cor.test(
            dat[, data.tree::FindNode(detStruct_original[[i]],
                                      paste0(v, "_original"))$scaleVarName],
            dat[, data.tree::FindNode(detStruct_short[[i]],
                                      paste0(v, "_short"))$scaleVarName])
        )
      }
    );
  names(corTestObjects[[behaviors[i]]]) <- determinants;

  ### Correlations between lean measures for this behavior
  fullVarNames <- detStruct_original[[i]]$Get('scaleVarName')
  fullVarNames <- fullVarNames[!is.na(fullVarNames)];
  corMatrixObjects_full[[behaviors[i]]] <-
    ufs::associationMatrix(dat, x = fullVarNames);
  
  ### Correlations between lean measures for this behavior
  leanVarNames <- detStruct_short[[i]]$Get('scaleVarName')
  leanVarNames <- leanVarNames[!is.na(leanVarNames)];
  corMatrixObjects_lean[[behaviors[i]]] <-
    ufs::associationMatrix(dat, x = leanVarNames);

  ufs::cat0("\n### Correlations between full and lean operationalisations\n\n");
  for (j in names(corTestObjects[[behaviors[i]]])) {
  ufs::cat0("\n- **", j, "**: ",
       ufs::formatCI(corTestObjects[[behaviors[i]]][[j]]$conf.int, noZero=TRUE),
       ", point estimate = ", ufs::formatR(corTestObjects[[behaviors[i]]][[j]]$estimate),
       ", n = ", corTestObjects[[behaviors[i]]][[j]]$parameter + 2);
  }

  ufs::cat0("\n\n\n### Correlation matrices using full operationalisations\n\n");

  sampleSize_full <- as.numeric(corMatrixObjects_full[[behaviors[i]]]$output$raw$n);
  sampleSize_full <- unique(sampleSize_full);
  sampleSize_full <- sampleSize_full[!is.na(sampleSize_full)];
  ufs::cat0("\n\nSample size = ", sampleSize_full, "\n\n");
  pander::pander(corMatrixObjects_full[[behaviors[i]]]);
  
  ufs::cat0("\n### Correlations matrices using lean operationalisations\n\n");

  sampleSize_lean <- as.numeric(corMatrixObjects_lean[[behaviors[i]]]$output$raw$n);
  sampleSize_lean <- unique(sampleSize_lean);
  sampleSize_lean <- sampleSize_lean[!is.na(sampleSize_lean)];
  ufs::cat0("\n\nSample size = ", sampleSize_lean, "\n\n");
  pander::pander(corMatrixObjects_lean[[behaviors[i]]]);
  
  ufs::cat0("\n### Q (effect size for difference between correlations)\n\n");
  
  fullZ <- ufs::convert.r.to.fisherz(corMatrixObjects_full[[i]]$output$raw$es);
  leanZ <- ufs::convert.r.to.fisherz(corMatrixObjects_lean[[i]]$output$raw$es);
  diffQ <- fullZ - leanZ;
  ZforQ <- diffQ / sqrt((1 / (sampleSize_full - 3)) +
                        (1 / (sampleSize_lean - 3)));
  pforQ <- 2*ifelse(ZforQ < 0, pnorm(ZforQ), 1-pnorm(ZforQ));
  
  qMatrix <- paste0("q=", ufs::formatR(diffQ), "\n",
                    ufs::formatPvalue(pforQ));
  qMatrix <- ifelse(grepl("NA", qMatrix),
                    "",
                    qMatrix);
  qMatrix <- matrix(qMatrix,
                    ncol=ncol(fullZ));

  rownames(qMatrix) <- determinants;
  colnames(qMatrix) <- determinants;

  pander::pander(qMatrix);
  
  all_p_values <- c(all_p_values, pforQ[!is.na(pforQ)]);
  
  ### Note - we may have to correct this, see https://twitter.com/matherion/status/1012705305237442562
}


```

## Distribution of *p* values for correlation comparisons

```{r}

hist(all_p_values);
ks.test(all_p_values, punif);
